...
void RtcDelayMs( TimerTime_t milliseconds )
{
    ...
    uint32_t refTicks = RTC_GetPeriod( );
    delayTicks = RtcMs2Tick( milliseconds );
    // Wait delay ms
    while( ( ( RTC_GetPeriod( ) - refTicks ) ) < delayTicks )
    {
        __NOP( );
    }
}
...
{
    RtcTimerContext.Time = ( uint32_t )RTC_GetPeriod( );
...
{
    return ( uint32_t )RTC_GetPeriod( );
}

uint32_t RtcGetTimerElapsedTime( void )
{
    return ( uint32_t)( RTC_GetPeriod( ) - RtcTimerContext.Time );
}
